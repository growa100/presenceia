  ...
  const renderContent = (text: string) => {
    ...
    lines.forEach((line, i) => {
      ...
      if (line.startsWith('## ')) {
        ...
        elements.push(<p key={i} className="text-white/60 text-base leading-relaxed mb-4">{line}</p>)
      }
    })
    return elements
  }
      ...
      <article className="pt-32 pb-24 max-w-3xl mx-auto px-6">
        ...
        <div className="mb-10">
          <div className="flex flex-wrap gap-2 mb-6">
            {post.tags.map(tag => (
              <span key={tag} className="font-mono text-xs text-brand/60 bg-brand/8 px-3 py-1 rounded-full flex items-center gap-1.5">
